...
- Added versioned staged repair proof plans with deterministic cheap-to-expensive validation, behavioral profile-command staging, safe environment-default resolution, checkout immutability checks, retention of every allowlisted required command, exact-command deduplication, explicit-only non-live subsumption with digest provenance, fail-closed canonical-gate stalls and runtime budgets, and bounded machine-readable traces bound to the exact validated head/base in repair reports and merge preflight.
